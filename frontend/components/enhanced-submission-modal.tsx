...
export function EnhancedSubmissionModal({ 
  trigger, 
  initialData, 
  onSubmissionComplete 
}: EnhancedSubmissionModalProps) {
  ...
  // Update submission data when modal opens or initialData changes
  useEffect(() => {
    if (open && initialData) {
      setSubmissionData({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/examples/enhanced`, {
        method: 'POST',
        headers: {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Enhanced Code-Switching Submission</DialogTitle>
          <DialogDescription>
